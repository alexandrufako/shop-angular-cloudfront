...
export class ProductsService extends ApiService {
  createNewProduct(product: Product): Observable<Product> {
    if (!this.endpointEnabled('bffApi')) {
      console.warn(
        'Endpoint "bffApi" is disabled. To enable change your environment.ts config'
      );
      return EMPTY;
    }

    const url = this.getUrl('bffApi', 'products');
  ...
  editProduct(id: string, changedProduct: Product): Observable<Product> {
    ...
    const url = this.getUrl('bffApi', `products/${id}`);
